    ...
    if (errorEmail.value.isEmpty && errorPassword.value.isEmpty) {
      try {
        ...
        if (response.status == 200) {
          ...
          await AppPreference.addStringToSF(Constants.apiToken, response.token ?? '');
          ...
          AppPreference.addStringToSF(
              Constants.password, response.data?.password ?? '');
          await AppPreference.addBoolToSF(
  ...
  Future<void> sendTokenData() async {
    ...
    if(userId.value.isNotEmpty){
      try {
        ...
        var request = {"_id": userId.value, "firebaseToken": '${fcmToken}', "notification": '${notification}'};
